...
class AprilTagPoseEstimator(Node):
    ...
    def image_callback(self, msg):
        ...
        self.detect_and_plot(gray, display_image)

        # Saving the images (**need to fix the frames)
        cv2.imwrite(f"/home/grlopi/slam_robot_workshop/images/frame_{self.frame_id}.png", display_image)
        self.frame_id += 1
    ...
    def detect_and_plot(self, cv_image, display_image):
        ...
        # Requires both tags (**need to add in a case for only 1 tag detected)
        if len(detections) < 2:
            return
